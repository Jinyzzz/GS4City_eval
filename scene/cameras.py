...
class Camera(nn.Module):
    ...
    def get_dual_language_features(self, sam_feature_dir, citygml_feature_dir, feature_level):
        """
        Load dual language features: SAM + CityGML

        Args:
            sam_feature_dir: Directory containing SAM features
            citygml_feature_dir: Directory containing CityGML features
            feature_level: Feature level to use (0/1/2/3)

        Returns:
            sam_feature: (C, H, W) SAM language feature map
            citygml_feature: (C, H, W) CityGML language feature map
            citygml_coverage: (H, W) CityGML coverage mask (True where CityGML is available)
        """
        # Load SAM features
        sam_feature, _ = self.get_language_feature(sam_feature_dir, feature_level)

        # Load CityGML features
        citygml_feature, _ = self.get_language_feature(citygml_feature_dir, feature_level)

        # Load CityGML coverage mask
        citygml_mask_path = os.path.join(citygml_feature_dir, self.image_name + '_mask.npy')
        citygml_coverage = torch.from_numpy(np.load(citygml_mask_path)).bool().cuda()

        return sam_feature, citygml_feature, citygml_coverage
